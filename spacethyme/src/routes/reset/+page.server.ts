import { deleteAllProcessedMaps, deleteUploadedCSVs, transformCsv } from '$lib/server/mapdata';
...
export const actions = {
    go: async ({  }) => {
        ...
        deleteAllProcessedMaps().then(() => {
            deleteUploadedCSVs().then(() => {
                transformCsv(filename, colmap, meta).then(() => {
                    console.log('transformed csv');
                }).catch((err) => {
                    console.error(err);
                });
            }).catch((err) => {
